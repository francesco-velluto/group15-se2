...
describe('getAllTickets function', () => {
    it('should return all tickets from db', async () => {
        db.query.mockResolvedValue({ 
            rows: [
                { number: 1, status: 1, service_id: 1, date: undefined, tag_name: 'tag1'},
                { number: 2, status: 0, service_id: 2, date: undefined, tag_name: 'tag2'},
                { number: 3, status: 2, service_id: 3, date: undefined, tag_name: 'tag3'},
                { number: 4, status: 1, service_id: 3, date: undefined, tag_name: 'tag4'}
            ]
        });
        const expectedTickets = [
            new Ticket(1, 1, date = undefined, 1, 'tag1'),
            new Ticket(2, 0, date = undefined,2, 'tag2'),
            new Ticket(3, 2, date = undefined,3, 'tag3'),
            new Ticket(4, 1, date = undefined,3, 'tag4')
        ];
...
describe('getLastTicketNumberByService function', () => {
  ...
  describe('getTicketDetails function', () => {
    it('should return ticket details', async () => {
        db.query.mockResolvedValue({ 
            rows: [ { number: 1, status: 1, service_id: 1, date: undefined, tag_name: 'tag1'},
        ]});
